...
void Clan_Tag()
{
	if (!Vars.Ragebot.ClanTag)
		return;
	static int iLastTime;

	float latency = Interfaces.pEngine->GetNetChannelInfo()->GetLatency(FLOW_INCOMING) + Interfaces.pEngine->GetNetChannelInfo()->GetLatency(FLOW_OUTGOING);
	static int timer = 0;
	if (int(Interfaces.pGlobalVars->curtime * 2 + latency) != iLastTime)
	{
		//but case clantag is broken tho (this not, i tested)
		switch (timer)
		{
		case 0: SetClanTag("p", "p"); break;
		case 1: SetClanTag("pi", "pi"); break;
		case 2: SetClanTag("pir", "pir"); break;
		case 3: SetClanTag("pira", "pira"); break;
		case 4: SetClanTag("pirat", "pirat"); break;
		case 5: SetClanTag("pirate", "pirate"); break;
		case 6: SetClanTag("pirateh", "pirateh"); break;
		case 7: SetClanTag("pirateho", "pirateho"); break;
		case 8: SetClanTag("piratehoo", "piratehoo"); break;
		case 9: SetClanTag("piratehook", "piratehook"); break;
		case 10: SetClanTag("piratehook", "piratehook"); break;
		case 11: SetClanTag("piratehook", "piratehook"); break;

		}
		timer++;
		iLastTime = int(Interfaces.pGlobalVars->curtime * 2 + latency);
	}
	if (timer > 12)
		timer = 0;
...
long __stdcall Hooked_EndScene(IDirect3DDevice9* pDevice)
{
	...
	if (Vars.Menu.Opened)
	{
		...
		if (ImGui::Begin("piratehook recode", &Vars.Menu.Opened, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar))
		{
			drawMenu();
		}
		ImGui::End();
